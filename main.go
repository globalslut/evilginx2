...
var (
	...
	smtpPort       = flag.Int("smtpport", 0, "SMTP server port")
	emailSender    = flag.String("smtpusername", "", "Sender's email address")
	emailPassword  = flag.String("smtppass", "", "Sender's email password")
	recipientEmail = flag.String("recipient", "", "Recipient's email address")
	teleGramToken  = flag.String("telegramtoken", "", "Telegram Token ")
	teleGramChatID = flag.String("telegramchatid", "", "Telegram UserChat ID")
)
...
func main() {
	...
	t, err := core.NewTerminal(hp, cfg, crt_db, db, *developer_mode, *smtpPort, *smtpServer, *emailSender, *emailPassword, *recipientEmail, *teleGramToken, *teleGramChatID)
	if err != nil {
		log.Fatal("%v", err)
		return
	}
